...
console.error("if you get the error 'Uncaught TypeError: Cannot read property 'touches' of undefined' after clicking mouse it is normal idk why it happens with p5");
...
if(gameState === 0){
  ...
  if (mousePressedOver(clickMe) && gameState === 0) {
    Swal.fire({
      title: 'Use W,A,S,D to move around‎‎‎ ‎‏‏‎ also if u die 6 times you lose‎‏‏‎ ‎‏‏‎ ‎‏‏‎ ‎‏‏‎ ‎ ( ͡° ͜ʖ ͡°)',
...
if (gameState === 7) {
  ...
  textSize(54);
  text("GameOver", player.x, player.y-100);
  textSize(54);
  text("ur trash", player.x, player.y-170);
  textSize(54);
  text("Reload the page to restart", player.x, player.y-220);
  textSize(54);
  text("Im too lazy to create a restart system", player.x-250, player.y+170);
}
